...
- **insurance_* fields**: Coverage metadata for billing workflows.
...
- **consent_* fields**: Audit of HIPAA consent and recording permissions.
- **metadata** (`jsonb`): Free-form integration metadata (EHR IDs, etc.).

## audio_recordings
- **id** (`uuid`): Primary key.
- **encounter_id** (`uuid`): Back-reference to encounter.
- **s3_* fields**: Location of encrypted audio artefacts.
